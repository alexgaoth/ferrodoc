...
fn parse_args(args: &[String]) -> Result<Option<Options>, String> {
    ...
    Ok(Some(Options {
        ...
        page: out.page,
        shaping: Shaping {
            github_input: out.deprecated.first().is_some_and(|name| !name.is_empty()),
            ..out.shaping
        },
...
struct Shaping {
    ...
    ascii: bool,
    /// Whether the **input** was spelled `markdown_github`. Pandoc's own
    /// deprecation notice says to use `gfm` and this maps it there — but
    /// pandoc does not, and numbers an ordered list differently for the
    /// alias. See [`ungithub_lists`]. It sits here rather than beside
    /// `from`, because what it does is reshape the document.
    github_input: bool,
}
